...
Proposed next version after 0.2.0: `0.2.1` (patch) unless a user-visible feature lands.
...
Push a `vX.Y.Z` tag only after manifests already contain `X.Y.Z`. The release workflow builds signed NSIS + updater artifacts, runs installer smoke, then publishes.
